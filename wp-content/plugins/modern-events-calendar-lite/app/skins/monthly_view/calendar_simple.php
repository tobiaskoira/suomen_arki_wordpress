    ...
    <?php
            ...
            if(isset($events[$today]) && count($events[$today]))
            {
                ...
                foreach($events[$today] as $event)
                {
                    ...
                    if(!$this->cache->has($event->data->ID.'_content'))
                    {
                        ...
                        else $event_content = isset($event->data->content) && trim($event->data->content) != '' ? mb_substr(strip_tags($event->data->content, '<style>'), 0, 320) : '';
                    ...
                    if($this->display_detailed_time && $this->main->is_multipleday_occurrence($event)) $tooltip_content .= '<div class="mec-event-detailed-time mec-tooltip-event-time mec-color">'.$this->icons->display('clock-o').' '.MEC_kses::element($this->display_detailed_time($event)).'</div>';
        ...
        for($list_day = 1; $list_day <= $days_in_month; $list_day++)
        {
            ...
            if($running_day == 6)
            {
                ...
                if((($day_counter+1) != $days_in_month) || (($day_counter+1) == $days_in_month && $days_in_this_week == 7))
                {
